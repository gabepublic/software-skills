...
description: Python type safety with type hints, generics, protocols, and strict type checking. Prefer ty (with uv) aligned with the FastAPI toolchain. Use when adding type annotations, implementing generic classes, defining structural interfaces, or configuring static analysis.
---
...
**Tooling:** Prefer **ty** for static analysis, managed with **uv** (`uv add --dev ty`, `uv run ty check`), matching the [FastAPI other-tools reference](../fastapi/references/other-tools.md) and [python-code-style](../python-code-style/SKILL.md). **mypy** and **pyright** remain valid if a project or organization already standardized on them.
...
- Configuring **ty** (or mypy / pyright) for strict checking
...
Run the type checker from the project environment:
...
Ad hoc check without adding `ty` to the project: `uvx ty check`. See [ty type checking](https://docs.astral.sh/ty/type-checking/) for behavior and options.
...
Run **`uv run ty check`** in CI to catch type errors early. Treat ty’s rule levels and [overrides](https://docs.astral.sh/ty/reference/configuration/#overrides) like gradual strictness for large codebases. With **mypy** or **pyright**, use their strict modes and incremental overrides instead.
...
**Ellipsis in protocols:** Using `...` as the body of a `Protocol` method (below) is standard typing practice. That is **not** the same as using `...` for required **FastAPI** parameters or **Pydantic** field defaults—the [Official FastAPI skill](../fastapi/SKILL.md) forbids the latter only; see *Do not use Ellipsis for path operations or Pydantic models* there.
...
### ty (preferred for FastAPI-aligned projects)

Configure ty under `[tool.ty]` in `pyproject.toml`. Full reference: [ty configuration](https://docs.astral.sh/ty/reference/configuration/).

```toml
# pyproject.toml
[tool.ty.environment]
python-version = "3.12"  # Optional; ty can infer from requires-python / the venv

[tool.ty.rules]
all = "error"  # Enabled rules report as errors; tune individual rules or overrides as needed
```

Example: relax specific rules under `tests/` while keeping the rest strict:
...
See [overrides](https://docs.astral.sh/ty/reference/configuration/#overrides) and [suppression comments](https://docs.astral.sh/ty/suppression/) to tighten coverage over time without blocking the whole repo.

### Alternative checkers (mypy, pyright)

If the team already uses **mypy** or **pyright**, keep one checker in CI and apply the same *strictness goals* below. Example **mypy** strict baseline:
...
Example **pyright** strict baseline:
...
### Strictness goals (any checker)

These targets apply whether you use **ty**, **mypy**, or **pyright**:
...
For **mypy**, enable stricter checking per module with `# mypy: strict` or `[[tool.mypy.overrides]]`. For **ty**, use `[tool.ty.rules]` and per-file overrides linked above.

## Best Practices Summary

1. **Annotate all public APIs** - Functions, methods, class attributes
2. **Use `T | None`** - Modern union syntax over `Optional[T]`
3. **Run strict type checking in CI** - Prefer `uv run ty check`; mypy/pyright equivalents if those are the project standard
