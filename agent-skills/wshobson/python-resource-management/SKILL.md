...
# Broad except here: rollback on any failure, then re-raise. See
# [python-error-handling](../python-error-handling/SKILL.md) (transaction scope).
@asynccontextmanager
async def database_transaction(conn: AsyncConnection):
    """BEGIN / COMMIT, or ROLLBACK on any exception, then re-raise."""
