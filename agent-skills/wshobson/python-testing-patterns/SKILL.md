...
**Tooling:** Use **uv** to run tools (`uv run pytest`, `uv add --dev pytest-cov`) and **HTTPX** for HTTP examples—consistent with [python-code-style](../python-code-style/SKILL.md) and the [FastAPI other-tools reference](../fastapi/references/other-tools.md) (HTTPX over Requests).
...
The `Database` class below is a fixture-mechanics demo, not a recommended DB layer. For FastAPI-aligned projects, use the SQLModel async session fixture in [fastapi-config-01/references/patterns.md](../fastapi-config-01/references/patterns.md) (Testing Pattern) as the canonical template.
...
class APIClient:
    """Simple API client using a pooled HTTPX client."""
...
def test_get_user_success():
    """Test successful API call by injecting a mock client."""
    ...
    client = APIClient("https://api.example.com", client=mock_client)
...
def test_get_user_not_found():
    """Test API call when raise_for_status fails (e.g. 404)."""
    request = httpx.Request("GET", "https://api.example.com/users/999")
...
def test_create_user():
    """Test user creation by injecting a mock client."""
    mock_client = Mock(spec=httpx.Client)
    mock_client.post.return_value.json.return_value = {"id": 2, "name": "Jane Doe"}
    ...
    assert mock_client.post.call_args.kwargs["json"] == user_data
```

Dependency injection of the HTTPX client (pattern shared with [python-design-patterns](../python-design-patterns/SKILL.md)) avoids patching module globals and matches the pooled-client guidance in [python-anti-patterns](../python-anti-patterns/SKILL.md).
...
Pick either **colocated tests** (next to the code) or a **parallel `tests/` tree** — both are valid. See [python-project-structure](../python-project-structure/SKILL.md) *Pattern 4: Test File Organization* for the trade-offs, and apply one approach consistently across the project.

Example parallel layout (common for larger FastAPI projects):
...
```bash
# Add once to the project (requires pyproject.toml / uv-managed env)
