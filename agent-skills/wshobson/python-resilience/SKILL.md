...
**Logging:** Examples use **structlog** to match [python-observability](../python-observability/SKILL.md). Where tenacity hooks need a stdlib-style logger (e.g. `before_sleep_log`), define a small callback that forwards to structlog instead of wiring two logging systems in parallel.
...
**Fail-safe scope:** Patterns like **`fail_safe`** (later in this skill) fit **non-critical** reads or enrichments where a **logged default** is acceptable. For **payments, auth, and data integrity**, prefer errors to surface—coordinate with [python-error-handling](../python-error-handling/SKILL.md) fail-fast boundaries.
...
**Note:** `Protocol` methods below use `...` as stubs—that is normal for `typing.Protocol` and is **not** the FastAPI/Pydantic “no `...` defaults” rule; see the [Official FastAPI skill](../fastapi/SKILL.md) section *Do not use Ellipsis for path operations or Pydantic models* for scope.
...
Degrade gracefully when **non-critical** operations fail. Do not wrap **authoritative** or **compliance-sensitive** flows this way; see **Fail-safe scope** under Core Concepts and [python-error-handling](../python-error-handling/SKILL.md).

**Relationship to [python-anti-patterns](../python-anti-patterns/SKILL.md) — *Bare Exception Handling*:** `fail_safe` deliberately catches `Exception` and returns a default. That is **not** the `except Exception: pass` anti-pattern as long as you (a) log the failure, (b) restrict usage to **non-critical** read paths (recommendations, enrichment, cache warm-up), and (c) never silence errors on authoritative writes, payments, auth, or audit-relevant code. Apply it at one clearly-named decorator boundary so reviewers can see the swallow on sight.
