...
### 5. Scope with resilience and resource-management skills

- **Fail-fast vs fail-safe:** [python-resilience](../python-resilience/SKILL.md) uses **defaults** when an operation is **optional** or **degradable** (e.g. empty recommendations). Keep **fail-fast** validation and **`raise`** for **money, security, invariants, and authoritative writes**. Do not decorate the same code path with both instincts unless the product explicitly defines which failures may degrade.

- **Broad `except`:** In **transactions** and **context managers**, `except Exception:` that **rolls back or cleans up** and then **`raise`** is intentional—any failure must undo the unit of work. See [python-resource-management](../python-resource-management/SKILL.md). That is **not** the anti-pattern of **`except Exception: pass`** or vague catches without re-raise.
...
Required fields do not need `Field(...)` (ellipsis). A type without a default is already required; put constraints on `Field` without using `...` as the default. This matches the [fastapi](../fastapi/SKILL.md) skill and keeps models consistent in FastAPI apps.
...
2. **Use specific exceptions** - Prefer `ValueError` / `TypeError` over vague handlers; avoid `except Exception: pass`. Transactional **`except Exception` + cleanup + `raise`** is intentional—see **Scope with resilience and resource-management skills** above.
