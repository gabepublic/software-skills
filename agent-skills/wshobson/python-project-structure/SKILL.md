...
**Known exception — API versioning.** FastAPI projects that follow [fastapi-config-01](../fastapi-config-01/SKILL.md) nest route modules under `app/api/v1/endpoints/` so that `v2/`, `v3/`, etc. can be added beside `v1/` without reshuffling the tree. That extra depth applies **only** to `app/api/`; the rest of the layout (`core/`, `models/`, `schemas/`, `services/`, `repositories/`) still stays flat per this skill.
